...
| 分类 | 含义 | 示例 |
|------|------|------|
| **signal** | 包含金融/交易内容 | "SPY 550 支撑"、"止损了"、"earnings next week" |
| **noise** | 无金融内容的闲聊 | "哈哈哈"、"ok"、"😅"、"晚安" |
| **empty** | 空消息（纯表情/附件） | |

**signal 检测规则：**
- Ticker 符号：`$SPY`、`NVDA`、`QQQ`、`SPX` 等
- 价格数字：`$550`、`7200点`、`15%` 等
- 交易术语：buy/sell、call/put、止损/止盈、support/resistance、CPI/Fed 等
- 中文股票名：英伟达、美光、特斯拉、伯克希尔 等

**低信息率 = (empty + noise) / 总消息数**
